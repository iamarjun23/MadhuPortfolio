import Link from "next/link";
import { defaultSiteSettings } from "@/schemas/settings";
import type { Contact, Settings } from "@/schemas";

type FooterProps = Readonly<{
  contact: Contact | null;
  settings: Settings;
}>;

type SocialName = "linkedin" | "instagram" | "youtube";

const SOCIAL_ICONS: Record<SocialName, string> = {
  linkedin:
    "M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z",
  instagram:
    "M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.058-1.28.072-1.689.072-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z",
  youtube:
    "M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z",
};

function SocialLink({
  href,
  name,
  label,
}: Readonly<{ href: string | null; name: SocialName; label: string }>) {
  if (!href) return null;

  return (
    <a
      className="public-footer__social"
      href={href}
      target="_blank"
      rel="noreferrer"
      aria-label={label}
      title={label}
    >
      <svg viewBox="0 0 24 24" width="17" height="17" fill="currentColor" aria-hidden="true">
        <path d={SOCIAL_ICONS[name]} />
      </svg>
    </a>
  );
}

export function Footer({ contact, settings }: FooterProps) {
  const year = new Date().getFullYear();
  const site = settings.site ?? defaultSiteSettings;
  const brand = site.brand ?? defaultSiteSettings.brand;
  const footer = site.footer ?? defaultSiteSettings.footer;

  return (
    <footer className="public-footer">
      <div className="wrap public-footer__grid">
        <div className="public-footer__brand">
          <span className="public-footer__label">{footer.eyebrow}</span>
          <Link className="brand" href="/" aria-label={brand.homeLabel}>
            <span className="brand__name">{brand.name}</span>
            <span className="brand__suffix">{brand.suffix}</span>
          </Link>
          {contact ? <p>{contact.footerTagline}</p> : null}
          {contact?.availableForFreelance ? (
            <span className="footer-status">
              <span className="footer-status__blip" aria-hidden="true" />
              {contact.footerStatus}
            </span>
          ) : null}
        </div>

        <div className="public-footer__craft">
          <h2 className="public-footer__label">{footer.craftEyebrow}</h2>
          <p className="public-footer__craft-lead">{footer.craftHeading}</p>
          <p className="public-footer__craft-note">{footer.craftDescription}</p>
          <ul className="public-footer__skills" aria-label="Editing skills">
            {footer.skills.map((skill) => (
              <li key={skill.id} className="public-footer__skill">
                <i aria-hidden="true">{skill.number}</i>
                {skill.label}
              </li>
            ))}
          </ul>
        </div>

        <nav className="public-footer__column" aria-label={footer.exploreHeading}>
          <h2 className="public-footer__label">{footer.exploreHeading}</h2>
          <ul className="public-footer__links">
            <li>
              <Link href="/#work">{footer.selectedWorkLabel}</Link>
            </li>
            <li>
              <Link href="/process#photobooth">{footer.photoboothLabel}</Link>
            </li>
            <li>
              <Link href="/#experience">{footer.experienceLabel}</Link>
            </li>
            <li>
              <Link href="/room">{footer.drawingRoomLabel}</Link>
            </li>
          </ul>
        </nav>

        {contact ? (
          <div className="public-footer__column">
            <h2 className="public-footer__label">{footer.contactHeading}</h2>
            <a className="public-footer__email" href={`mailto:${contact.email}`}>
              {contact.email}
            </a>
            <span className="public-footer__location">{contact.location}</span>
            <div className="public-footer__socials" aria-label="Social links">
              <SocialLink href={contact.socials.linkedin} name="linkedin" label="LinkedIn" />
              <SocialLink href={contact.socials.instagram} name="instagram" label="Instagram" />
              <SocialLink href={contact.socials.youtube} name="youtube" label="YouTube" />
            </div>
          </div>
        ) : null}
      </div>

      <div className="wrap public-footer__bottom">
        <span>
          {footer.copyrightPrefix} {year} {site.ownerName ?? defaultSiteSettings.ownerName}
        </span>
        <span>{footer.closingLine}</span>
      </div>
    </footer>
  );
}
